...
def train_model(model: ChessModel, positions: torch.Tensor, valid_moves: torch.Tensor) -> Dict:
    ...
    for batch_num, (train_positions, train_valid_moves) in enumerate(train_dataloader):
        ...
        move_logits = model(train_positions)
    ...
    with torch.no_grad():
        for test_positions, test_valid_moves in test_dataloader:
            test_move_logits = model(test_positions)
